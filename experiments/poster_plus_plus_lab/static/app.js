...
const fields = {
  ...
  pretrainState: document.querySelector("#pretrainState"),
  upstreamState: document.querySelector("#upstreamState"),
...
async function checkService() {
  try {
    ...
    fields.evidence.textContent = `无法连接实验服务：${error.message}`;
  }
}

function renderModelState(modelData) {
  fields.runtimeStatus.textContent = modelData.runtimeStatus || "-";
  fields.checkpointState.textContent = modelData.weightsPresent
    ? "ready"
    : `missing: ${modelData.checkpointPath}`;
  fields.pretrainState.textContent = modelData.pretrainPresent
    ? "ready"
    : `missing: ${(modelData.missingPretrain || []).join(", ") || modelData.pretrainDir}`;
  fields.upstreamState.textContent = modelData.upstreamPresent
    ? "ready"
    : `missing: ${modelData.upstreamPath || "POSTER_PP_UPSTREAM"}`;

  if (!modelData.runtimeReady) {
    fields.evidence.textContent =
      `POSTER++ 资源未就绪：${modelData.runtimeStatus}。` +
      "请按 models/poster-plus-plus/README.md 放置 checkpoint、pretrain 和 upstream。";
  }
}
...
function renderResult(payload) {
  ...
  fields.confidence.textContent = formatNumber(payload.confidence, 4);
  fields.runtimeStatus.textContent = "ready";
  fields.preprocessStatus.textContent = payload.features?.preprocess?.mode || "cropped";
